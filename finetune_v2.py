...
#### CLASSIC PROMPT ####
def make_conversational_prompt(history, response):
    prompt = f"""Dưới đây là cuộc hội thoại giữa người và trợ lý ảo AI (Chatbot). Chatbot sẽ phản hồi những câu hỏi hay yêu cầu từ ngưòi bằng cách đưa ra câu trả lời chính xác và hữu ích.
### Conversation:
{history} {response}"""
    return prompt

def generate_prompt(data_point):
    history, response = data_point["history"], data_point["response"]
    prompt = make_conversational_prompt(history, response)
    return prompt
#### CLASSIC PROMPT ####
...
def train(
    ...
    def tokenize_function(data_point):
        ...
        num_input_tokens = len(result["input_ids"]) - response_length
        for token in range(0, num_input_tokens):
            result["labels"][token] = -100
    ...
    if val_set_size > 0:
        ...
        train_data = data["train"].shuffle(seed=71).map(tokenize_function)
        val_data = None
    ...
    if ddp: # huấn luyện đa GPUs
        print(f">>> Training multi-GPU, world size = {world_size}")
